...
def load_game():
    ...
    pygame.font.init()
    # Initialize mixer module for sound
    pygame.mixer.init()
    ...
    sounds['point_scored'] = pygame.mixer.Sound('./sounds/score.wav')
    ...
    player2 = Paddle(VIRTUAL_WIDTH - 15, VIRTUAL_HEIGHT - 50, 3, 20)
    ...
    ball = Ball(VIRTUAL_WIDTH / 2 - 2, VIRTUAL_HEIGHT / 2 - 2, 3, player1, player2)
...
def update(dt):
    ...
    #Update ball only if game state is 'play'
        ...
        MISS_PROBABILITY = 0.1 # not too beatable ;)

        if random.random() > MISS_PROBABILITY:
            if ball.y < player2.y:
                player2.moveUp()
            elif ball.y > (player2.y + player2.height):
                player2.moveDown()
